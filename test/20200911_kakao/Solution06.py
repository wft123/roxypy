...
def solution(board, r, c):
    answer = 0
    move_board(board, (r, c), 0, (-1, -1))
    return min_cnt


def move_board(board, cur, count, pivot):
    n = len(board)
    x = cur[0]
    y = cur[1]
    cur_card = board[x][y]
    piv_card = 0 if pivot[0] == -1 and pivot[1] == -1 else board[pivot[0]][pivot[1]]

    if cur_card:
        count += 1
        if piv_card == 0:
            pivot = (x, y)
        elif cur_card == piv_card:
            board[x][y], board[pivot[0]][pivot[1]] = 0, 0
            pivot = (-1, -1)

    if x > 0: move_board(board, (x - 1, y), count + 1, pivot)
    if x < n-1: move_board(board, (x + 1, y), count + 1, pivot)
    if y > 0: move_board(board, (x, y-1), count + 1, pivot)
    if y < n-1: move_board(board, (x, y + 1), count + 1, pivot)

    if not x: move_board(board, (0, y), count + 1, pivot)
    if x != n-1: move_board(board, (n - 1, y), count + 1, pivot)
    if not y: move_board(board, (x, 0), count + 1, pivot)
    if y != n-1: move_board(board, (x, n-1), count + 1, pivot)
...
param = [[1,0,0,3],[2,0,0,0],[0,0,0,2],[3,0,1,0]]
print(solution(param, 1, 0))

# print(is_finish(param))
